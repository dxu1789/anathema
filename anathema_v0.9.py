...
0.9: cross-platform issues: using os.path.join, uses winsound only for windows
...
def parseline(line):
    ...
    if len(parsedline) > 0:
        return line # parsedline #parsedline[0]
    return ""
...
def savegame(savename):
    ...
    if len(savename) <= 0:
        ...
        return
    try:
        savetarget = saves_directory + "\\" + savename + ".txt"
...
sound_endround = "sounds\\button.wav"  # "sounds\\redalert.wav"
rulesfile = "rules.txt"
